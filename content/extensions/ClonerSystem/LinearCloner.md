...
![idpic1](/img/extensions/clonerSystem/linearCloner.jpg "LinearCloner's с разными параметрами преобразования")

LinearCloner клонирует и распределяет данные сетки линейным способом. Если предусмотрено более одной сетки, то соответствующие клоны будут размещены последовательно один за другим. The LinearCloner can instantiated in two different interpolation-modes: absolute and relative. In the first mode the values of the input parameters (scale/rotation/position) can be seen as the difference from the first to the last clone, whereas in the relative mode those values are the difference from clone to clone. 
